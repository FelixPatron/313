...
client.on('guildMemberAdd', member => {
  let rol = "958802412699787324"
  member.roles.add(rol)
  client.channels.cache.get('958837362773885018').send(`${member} **Kişisine <@&${rol}> Rolünü Verdim, Hoşgeldin.**`)
})
...
client.on('ready', () => {
      ...
      var actvs = [
        `${prefix}yardım ${client.guilds} sunucuyu`,
        `${prefix}yardım ${client.users} Kullanıcıyı`, 
        `${prefix}yardım`
    ];
    ...
    setInterval(() => {
      ...
      console.log (`Sunucular          : ${client.guilds}`);
      console.log (`Kullanıcılar       : ${client.users}`);
...
client.on('guildMemberAdd', async member => {
  ...
  client.channels.cache.get(kanal).send(`
    **${member}(\`${member.id}\`) Sunucumuza hoşgeldin dostum. Seninle birlikte \`${member.guild.memberCount}\` kişi olduk. Ses teyit odalarına geçerek kayıt olabilirsin. Ayrıca tagımızı alarak bize destek olabilirsin \`tag-gelicek\` :tada:**
    \n
...
member.roles.add('958802412699787324')
})
...
let bal  = db.fetch(`davet_${invite.inviter.id + member.guild.id}`)
   member.guild.channels.get(channel).send(`:inbox_tray: ** <@${member.id}> Katıldı**; Davet Sahibi **${davetçi.tag}** (`+'**'+bal+'** Davet)')
  })
